...
if [[ ! -d "$MODEL_PATH/FL2VA" || ! -d "$MODEL_PATH/transformer" ]]; then
    echo "MODEL_PATH must point to a MiniMax-H3 repo root containing FL2VA/ (fused rollout) and transformer/ (diffusers actor) (got: '$MODEL_PATH')" >&2
    exit 1
fi
...
MAX_PROMPT_EMBEDS=${MAX_PROMPT_EMBEDS:-6400}

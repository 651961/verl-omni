...
Saved rollout and validation JSONL rows retain `extra_info.index` from the
corresponding train or test dataset, so generated media can be traced back to
its source record. Repeated generations share their source index; media filenames
are dump positions, not dataset indices. An absent dataset index is recorded as
`null` in validation output.

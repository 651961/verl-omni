...
class BaseRayDiffusionTrainer(ABC):
    ...
    def _log_rollout_data(
        ...
        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
            ...
            # Preserve dataset identity in the same order as the generated samples.
            extra_infos = batch.non_tensor_batch.get("extra_info")
            if extra_infos is not None:
                indices = [info.get("index") if isinstance(info, dict) else None for info in extra_infos]
                reward_extra_infos_to_dump["extra_info"] = [
                    {"index": index.item() if isinstance(index, np.generic) else index} for index in indices
                ]
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            for info in test_batch.non_tensor_batch.get("extra_info", [None] * len(test_batch)):
                index = info.get("index") if isinstance(info, dict) else None
                sample_extra_infos.append({"index": index.item() if isinstance(index, np.generic) else index})
        ...
        if val_data_dir:
            # Dataset identity belongs in the dump, not in reward metric aggregation.
            extra_infos_to_dump = {**reward_extra_infos_dict, "extra_info": sample_extra_infos}
            self._dump_generations(
                inputs=sample_inputs,
                outputs=sample_outputs,
                gts=sample_gts,
                scores=sample_scores,
                reward_extra_infos_dict=extra_infos_to_dump,
